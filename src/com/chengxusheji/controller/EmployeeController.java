...
public class EmployeeController extends BaseController {
	...
	public void add(@Validated Employee employee, BindingResult br,
		...
		try {
			employee.setPhoto(this.handlePhotoUpload(request, "photoFile"));
	...
	/*ajax方式按照查询条件分页查询员工信息信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String employeeNo,String name,String department,String birthDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Employee> employeeList = employeeService.queryEmployee(employeeNo, name, department, birthDate, page);
	    /*计算总的页数和总的记录数*/
	    employeeService.queryTotalPageAndRecordNumber(employeeNo, name, department, birthDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String employeeNo,String name,String department,String birthDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (employeeNo == null) employeeNo = "";
		if (name == null) name = "";
		if (department == null) department = "";
		if (birthDate == null) birthDate = "";
		List<Employee> employeeList = employeeService.queryEmployee(employeeNo, name, department, birthDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    employeeService.queryTotalPageAndRecordNumber(employeeNo, name, department, birthDate);
	    /*获取到总的页码数目*/
	    int totalPage = employeeService.getTotalPage();
	...
	public void update(@Validated Employee employee, BindingResult br,
		...
		if(!photoFileName.equals("upload/NoImage.jpg"))employee.setPhoto(photoFileName); 
	...
	/*按照查询条件导出员工信息信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String employeeNo,String name,String department,String birthDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(employeeNo == null) employeeNo = "";
        if(name == null) name = "";
        if(department == null) department = "";
        if(birthDate == null) birthDate = "";
        List<Employee> employeeList = employeeService.queryEmployee(employeeNo,name,department,birthDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Employee信息记录"; 
        String[] headers = { "员工编号","姓名","性别","员工照片","所在部门","出生日期","联系电话","联系地址"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<employeeList.size();i++) {
        	Employee employee = employeeList.get(i); 
        	dataset.add(new String[]{employee.getEmployeeNo(),employee.getName(),employee.getSex(),employee.getPhoto(),employee.getDepartment(),employee.getBirthDate(),employee.getTelephone(),employee.getAddress()});
        }
        /*
