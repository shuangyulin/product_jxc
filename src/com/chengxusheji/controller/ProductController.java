...
public class ProductController extends BaseController {
	...
	public void list(String productNo,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String madeDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (productNo == null) productNo = "";
		if (productName == null) productName = "";
		if (madeDate == null) madeDate = "";
		if(rows != 0)productService.setRows(rows);
		List<Product> productList = productService.queryProduct(productNo, productClassObj, productName, madeDate, page);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(productNo, productClassObj, productName, madeDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String productNo,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String madeDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (productNo == null) productNo = "";
		if (productName == null) productName = "";
		if (madeDate == null) madeDate = "";
		List<Product> productList = productService.queryProduct(productNo, productClassObj, productName, madeDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    productService.queryTotalPageAndRecordNumber(productNo, productClassObj, productName, madeDate);
	    /*获取到总的页码数目*/
	    int totalPage = productService.getTotalPage();
	...
	public void OutToExcel(String productNo,@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String madeDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(productNo == null) productNo = "";
        if(productName == null) productName = "";
        if(madeDate == null) madeDate = "";
        List<Product> productList = productService.queryProduct(productNo,productClassObj,productName,madeDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Product信息记录"; 
        String[] headers = { "商品编号","商品类别","商品名称","产品图片","商品库存","市场价","产地","规格","生产日期","有效期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<productList.size();i++) {
        	Product product = productList.get(i); 
        	dataset.add(new String[]{product.getProductNo(),product.getProductClassObj().getProductClassName(),product.getProductName(),product.getProductPhoto(),product.getCount() + "",product.getPrice() + "",product.getMadeAddress(),product.getGuige(),product.getMadeDate(),product.getYxq()});
        }
        /*
