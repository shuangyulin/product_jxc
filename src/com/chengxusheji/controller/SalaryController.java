...
public class SalaryController extends BaseController {
	...
	public void list(@ModelAttribute("employeeObj") Employee employeeObj,String year,String month,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Salary> salaryList = salaryService.querySalary(employeeObj, year, month, page);
	    /*计算总的页数和总的记录数*/
	    salaryService.queryTotalPageAndRecordNumber(employeeObj, year, month);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("employeeObj") Employee employeeObj,String year,String month,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (year == null) year = "";
		if (month == null) month = "";
		List<Salary> salaryList = salaryService.querySalary(employeeObj, year, month, currentPage);
	    /*计算总的页数和总的记录数*/
	    salaryService.queryTotalPageAndRecordNumber(employeeObj, year, month);
	    /*获取到总的页码数目*/
	    int totalPage = salaryService.getTotalPage();
	...
	public void OutToExcel(@ModelAttribute("employeeObj") Employee employeeObj,String year,String month, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(year == null) year = "";
        if(month == null) month = "";
        List<Salary> salaryList = salaryService.querySalary(employeeObj,year,month);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Salary信息记录"; 
        String[] headers = { "工资id","员工","年份","月份","考勤扣除","保险扣除","工资金额"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<salaryList.size();i++) {
        	Salary salary = salaryList.get(i); 
        	dataset.add(new String[]{salary.getSalaryId() + "",salary.getEmployeeObj().getName(),salary.getYear(),salary.getMonth(),salary.getAttendanceMoney() + "",salary.getSecureMoney() + "",salary.getSalaryMoney() + ""});
        }
        /*
